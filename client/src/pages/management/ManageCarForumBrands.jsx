...
import {
  ...
  updateCarForumBrand,
  deleteCarForumBrand,
} from "../../services/api/carForumBrandService";
import { useAuth } from "../../contexts/AuthContext";

export default function ManageCarForumBrands() {
  ...
  const loadBrands = async () => {
    try {
      const data = await getCarForumBrands();
      setBrands(data || []);
    } catch (err) {
      console.error("Failed to fetch forum brands:", err);
      setError("Failed to load forum brands");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedBrandId !== null) {
        await updateCarForumBrand(selectedBrandId, {
          id: selectedBrandId,
          name: formName,
        });
      } else {
        await createCarForumBrand({ name: formName });
      }
      ...
      console.error("Error saving forum brand:", err);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this forum brand?")) return;
    try {
      await deleteCarForumBrand(id);
      await loadBrands();
    } catch (err) {
      ...
      <main className="flex-1 px-4 py-10 flex justify-center">
        <div className="w-full max-w-3xl bg-white rounded-xl shadow-lg border border-gray-300 p-8">
          <h2 className="text-2xl font-semibold mb-6 text-center">
            Manage Car Forum Brands
          </h2>

          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            <input
              type="text"
              placeholder="Forum Brand Name"
